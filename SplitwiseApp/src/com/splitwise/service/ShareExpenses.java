...
import com.splitwise.model.OwedUser;
import com.splitwise.model.User;

public class ShareExpenses {

	List<User> users;
	// u1 -> relation -> (u2 , u2+balance)
		...
		if (user.isPresent()) {
			...
			for (User owedUser : owedUsers) {
				if (expensesMap.get(owedUser) != null && owedUser != user.get()) {
					...
					if (remaining > 0) {
						// u1 -> owes(give money) ->u2 : remaining
						...
						// u2 -> owes(give money) ->u1 : remaining
	...
	public void splitExactExpenses(String userName, double totalAmount, List<User> owedUsers,
			HashMap<User, Double> owedAmountMap) {
		Optional<User> user = getUser(userName);
		if (user.isPresent()) {
			HashMap<User, OwedUser> userSpecificMap;
			if (expensesMap.get(user.get()) != null) {
			...
			for (User owedUser : owedUsers) {
				if (expensesMap.get(owedUser) != null && owedUser != user.get()) {
					double remaining = expensesMap.get(owedUser).get(user.get()).getBalance()
							- owedAmountMap.get(owedUser);
					if (remaining > 0) {
						expensesMap.get(owedUser).get(user.get()).setBalance(remaining);
					} else if (remaining < 0) {
						expensesMap.get(owedUser).remove(user.get());
	...
	public void splitPercentageExpenses(String userName, double totalAmount, List<User> owedUsers,
			HashMap<User, Integer> owedPercentageMap) {
		Optional<User> user = getUser(userName);
		if (user.isPresent()) {
			HashMap<User, OwedUser> userSpecificMap;
			if (expensesMap.get(user.get()) != null) {
			...
			for (User owedUser : owedUsers) {
				if (expensesMap.get(owedUser) != null && owedUser != user.get()) {
					double remaining = expensesMap.get(owedUser).get(user.get()).getBalance()
							- getPercentageAmount(totalAmount, owedPercentageMap.get(owedUser));
					if (remaining > 0) {
						expensesMap.get(owedUser).get(user.get()).setBalance(remaining);
					} else if (remaining < 0) {
						expensesMap.get(owedUser).remove(user.get());
					...
					if (userSpecificMap.get(owedUser) != null) {
						...
						OwedUser owedUserObj = new OwedUser(owedUser,
								getPercentageAmount(totalAmount, owedPercentageMap.get(owedUser)));
						userSpecificMap.put(owedUser, owedUserObj);
					}
				}
			...
			if (expensesMap.get(showUser.get()) != null) {
				...
				for (Map.Entry<User, OwedUser> entry : owedUsers.entrySet()) {
					...
					if (!entry.getKey().getUserName().equals(showUser.get().getUserName()))
						System.out.println(entry.getKey().getUserName() + " Owes " + showUser.get().getUserName() + ": "
				...
				if (expensesMap.get(user) != null && user != showUser.get()) {
					expensesMap.get(user).forEach((user1, owedUser) -> {
						if (user1.getUserName().equals(showUser.get().getUserName())
							...
							isUserHasData.set(true);
							System.out.println(owedUser.getUser().getUserName() + " Owes " + user.getUserName() + ": "
									+ owedUser.getBalance());
						}
					});
				}
